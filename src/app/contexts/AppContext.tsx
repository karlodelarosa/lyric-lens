...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  React.useEffect(() => {
    const onStorage = (event: StorageEvent) => {
      ...
      try {
        setLiveState((prev) => ({ ...prev, ...JSON.parse(event.newValue || "{}") }));
      } catch {
  ...
  const updateSetlist = (id: string, updates: Partial<Setlist>) => {
    setSetlists(setlists.map(sl => sl.id === id ? { ...sl, ...updates } : sl));
  };
  ...
  const updateSchedule = (id: string, updates: Partial<Schedule>) => {
    setSchedules(schedules.map(sch => sch.id === id ? { ...sch, ...updates } : sch));
  };
